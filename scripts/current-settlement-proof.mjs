...
const ROUNDS = {
  eligible: {
    ...
    project: "eligible-current",
    evidence: [
      "https://raw.githubusercontent.com/NicolasGarzon0/ETHWalletInsight/main/README.md",
      "https://raw.githubusercontent.com/gmh5225/awesome-crypto-wallet-address/master/README.md",
...
async function runRound(name) {
  ...
  const judgeHash = await writeAndWait("judge_case", [caseId], 0n, 180, 8000);
  const docket = asRecord(await call("get_case", [caseId]));
  const bond = asRecord(await call("get_bond_status", [caseId]));
  const eligible = asRecord(await call("is_eligible", [spec.wallet]));
